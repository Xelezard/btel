use btel::{view, App, Display, Highlight, Mode, Theme};
use tree::Root;
...
pub fn render(f:&mut  Frame<'_,CrosstermBackend<std::io::Stdout>>, app: App,scroll: &mut usize,scroll_x: &mut usize,opened_folder: &mut Option<String>,files_in_folder: &Vec<String>,targets_folder: &bool,folder_cursor: &usize,folder_error: &Option<String>,highlight_config: &mut Vec<(String,Highlight)>,theme: &Theme,config_tree: &mut Root<String>,forced_save: &mut bool) {
    ...
    .constraints(
    match app.mode {
        Mode::Edit | Mode::View => {[Constraint::Min(4),Constraint::Length(3),Constraint::Percentage(0)].as_ref()},
    ...
    let text_spans = if app.mode != Mode::View{
        ...
        let text = app.textbox.view.iter().map(|v|v.iter().map(|c|c.to_string()).collect::<String>()).collect::<Vec<String>>();
        let text = text.join("\n");
        let mut  text_spans = view::view(text,&app.textbox.view_info);
        if app.textbox.view.len() <= (*scroll + (chunks[0].height as usize)+2) {
            let on_screnn = Text::from(text_spans.lines.drain(*scroll..).as_slice().iter().map(|l|l.clone()).collect::<Vec<Spans>>());
            text_spans = on_screnn
        } else {
            let on_screen = Text::from(text_spans.lines.drain(*scroll..(*scroll + (chunks[0].height as usize) -1)).as_slice().iter().map(|l|l.clone()).collect::<Vec<Spans>>());
    ...
    let input_block = Block::default().borders(Borders::ALL).border_type(theme.border_type).title(Spans::from(vec![if  *forced_save{Span::styled("*", Style::default().fg(Color::Red).add_modifier(Modifier::CROSSED_OUT | Modifier::DIM))} else if app.textbox.saved {Span::raw("")} else {Span::styled("*",Style::default().fg(Color::Red))},Span::raw(app.file_name)])).border_style(match app.mode {Mode::Edit | Mode::View => Style::default().fg(theme.target), _ => Style::default().fg(theme.no_target).add_modifier(Modifier::DIM)});
    ...
    if matches!(app.mode,Mode::Edit|Mode::Find(_,_)|Mode::View) {
