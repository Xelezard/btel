...
fn theme(config_tree: &mut Root<String>) -> Theme{
    if let Ok(root) = config_tree.get_child("theme") {
        return match root.get_value().unwrap().as_str() {
            ...
            _ => Theme {border_type: BorderType::Plain,target: Color::White,no_target: Color::DarkGray}
        };
    }
    Theme {border_type: BorderType::Plain,target: Color::White,no_target: Color::DarkGray}
}
                ...
                match mode {
                    ...
                    Mode::Find(x,y) => {
                        ...
                        match key.code {
                            ...
                            KeyCode::Enter => {if let Some((x_cursor,y_cursor)) = find(&command, &textbox.input.iter().map(|v|v.iter().collect::<String>()).collect::<Vec<String>>(),x,y){textbox.vert_cursor = y_cursor;textbox.edit_cursor = x_cursor;mode = Mode::Find(x_cursor +1, y_cursor)} else if x == 0 && y == 0{line_name = String::from("Pattern not found")}else {mode = Mode::Find(0, 0)}}
...
fn exc_command(command: &mut String,output:&mut String,mode: &mut Mode,display: &mut Display,textbox:&mut TextBlock,file_name: &mut String,line_name:&mut String,commands: &Vec<Extern>,scroll_x: &mut usize,scroll_y: &mut usize,opened_folder: &mut Option<String>,files_in_folder: &mut Vec<String>,folder_error: &mut Option<String>,forced_save: &mut bool) {
    ...
    match btel_command {
        ...
        BtelCommand::Save => {pieces.push("");save(&pieces[1].to_string(),file_name,&textbox.input,&mut textbox.saved);*forced_save = false}
        BtelCommand::Edit => {*mode = Mode::Edit},
        BtelCommand::ForceSave => {textbox.saved = true;*forced_save = true},
...
fn set_from_btel_vars(vars: BtelVars,textbox:&mut textblock::TextBlock,output:&mut String,mode: &mut Mode,line_name:&mut String,file_name: &mut String,scroll_x: &mut usize,scroll_y: &mut usize,display: &mut Display) {
    *textbox = TextBlock {
        input: vars.input.iter().map(|s|s.chars().collect::<Vec<char>>()).collect(),
...
pub fn gen_stat<'a>(app: &App,theme: &Theme,vert_cursor:&usize,edit_cursor:&usize,conf: &'a mut Root<String>) -> Tabs<'a> {
    ...
    tui::widgets::Tabs::new(bar).block(Block::default().borders(Borders::ALL).border_type(theme.border_type).border_style(Style::default().add_modifier(Modifier::DIM).fg(theme.no_target)))
}
